...
> В любой экосистеме ROS существует мастер (и он единственный), который работает с узлами. Фактически, он полностью органиует их.  
...
Далее можно показать сообщения, которые идет через этот топик. Для завершения нажмите Ctrl+C.
...
##### > Найдите в справке утилиты `rostopic` команду измерения частоты публикации и измерьте частоту публикации в топик `/chatter`
...
Type: std_msgs/String
```
В типе можно видет имя пакета типа `std_msgs` и само название `String`. Чтобы просмотреть, что содержит данный тип сообщения, воспользуемся командой
```bash
...
##### > Попробуйте запустить узел пакета `rospy_tutorials` с именем `listener`, но присвоить имя `__name:=listener2`. Проверьте с помощью `rqt_graph`, что оба `listener` подключилиь на общий топик.
...
- Познакомились с утилитами `roscore`, `rosrun`, `rosnode`, `rostopic`, `rosmsg`, `rqt_graph`.
