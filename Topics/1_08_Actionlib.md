...
Второй сектор `result` - это информация, коорая будет передана от `ActionServer` к `ActionClient` по окончанию обработки.

Третий сектор `feedback` - информация, которая с какой-то периодичность будет передаваться от `ActionServer` к `ActionClient` в ходе работы.
