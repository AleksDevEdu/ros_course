...
- Если говорить об использовании `git`, то рекомендуется размещать в репозитории именно содержимое самого пакета (то есть того, что находится в папке src внутри catkin_ws), таким образом репозиторий будет представлять из себя пакет ROS, что в результате позволит легко импортировать пакет в ws на других машинах; размещение в `git` всего ws является плохой практикой и не рекомендуется к использованию.