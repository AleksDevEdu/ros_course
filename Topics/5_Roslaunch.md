...
- output - режим вывода информации, есть варианты `screen` и `log`
...
> Попробуйте написать свой launch-файл с таким же содержанием и запустить его. Для этого нужно в пакете создать папку `launch` и в ней создать файл с расширением launch. С помощью утилиты roslaunch запустите файл из своего пакета и убедитесь, что все работает. А когда пройдёте стадии гнева и отчаяния, проверьте, чтобы имя пакета было ваше собственное (= 
...
Допустим мы хотим запустить узлы в одном пространстве имен, так как они выполняют определенную задачу (являются подсистемой). Можно это сделать красиво с помощью тэга `<group>`.
