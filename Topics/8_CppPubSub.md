...
Далее указываем cmake о том, что мы планируем создавать исполняемый файл макросом `add_executable()`. В нем первый аргумент - название бинарного файла (который будет создан), второй - список файлов исходных текстов (как удачно совпало, что мы для этого опреелили переменную =)).
```cmake
