...
> Для дальнейшей работы требуется установить примеры работы с роботом `turtlebot` командой `sudo apt install ros-kinetic-turtlebot-gazebo`.
...
> В данной системе вместо простых узлов (Nodes) используются Nodelets - это схожий механизм, но имеет более специфичный интерфейс, разбираться с ним пока не будем, но он имеет свои преимущества =)
...
Пока этого достаточно, чтобы понять, что у нас есть робот в симуляторе, у него есть датчикик и им можно управлять, но вот системы управления еще пока нет. Это также видно и по тому, что узлы по работе с Kinect и эмуляции лидара ни к чему не привязаны, а вот топики в `/mobile_base/*` связывают управляющие команды (`/cmd_vel_mux`) и Gazebo (`/gazebo`), чтобы мы задавали управление, а робот через эту цепочку двигался внутри симулятора =)
...
Как видно, запущенный launch-файл позволяет управлять роботом с помощью клавиш `j`, `i` и др. **Время гонять!**
